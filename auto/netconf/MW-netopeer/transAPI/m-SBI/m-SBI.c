...
int callback_ms_notify(const char *filepath, xmlDocPtr *edit_config, int *exec){
	...
	fp = fopen ( "/etc/toMW.txt" , "rb" );
	if( !fp ) perror("/etc/toMW.txt"),exit(1);
...
struct transapi_file_callbacks file_clbks = {
	.callbacks_count = 1,
	.callbacks = {
		{.path = "/etc/toMW.txt", .func = callback_ms_notify}
	}
};
