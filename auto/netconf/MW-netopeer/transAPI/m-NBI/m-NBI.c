...
int callback_mn_m_NBI(void **data, XMLDIFF_OP op, xmlNodePtr old_node, xmlNodePtr new_node, struct nc_err **error) {
	...
	if (op & (XMLDIFF_ADD | XMLDIFF_MOD)) {
		...
		//Todo: Use google protobuf or binn for packaging configuration
 ...
 * }
 */
int callback_mn_notify(const char *filepath, xmlDocPtr *edit_config, int *exec){
	...
	fp = fopen ( "/etc/toCTRL.txt" , "rb" );
	if( !fp ) perror("/etc/toCTRL.txt"),exit(1);
...
struct transapi_file_callbacks file_clbks = {
	.callbacks_count = 1,
	.callbacks = {
		{.path = "/etc/toCTRL.txt", .func = callback_mn_notify}
	}
};
