...
/**
 * @brief This callback will be run when node in path /test:test/test:ap_id changes
 *
...
int callback_test_test_test_ap_id(void **data, XMLDIFF_OP op, xmlNodePtr old_node, xmlNodePtr new_node, struct nc_err **error) {

	if (op & (XMLDIFF_ADD | XMLDIFF_MOD)) {

		char* tmp1,*tmp2;
		tmp1 = new_node->children->content;	
		tmp2 = new_node->next->children->content;
...
struct transapi_data_callbacks clbks =  {
	...
	.data = NULL,
	.callbacks = {
		{.path = "/test:test/test:ap_id", .func = callback_test_test_test_ap_id}
	}
};
...
int example_callback1(const char *filepath, xmlDocPtr *edit_config, int *exec) {
	...
	if( !fp ) perror("/etc/my_cfg_file1.txt"),exit(1);
	...
	asprintf(&str, "<event>%s</event>", buffer);
